...
fun EasySshApp() {
    ...
    // 1. Setup Drawer State and Coroutine Scope
    val drawerState = rememberDrawerState(initialValue = DrawerValue.Closed)
    val scope = rememberCoroutineScope()

    // 2. Define all 8 screens for the side menu
    ...
    // 3. Wrap Scaffold with ModalNavigationDrawer
    ModalNavigationDrawer(
        drawerState = drawerState,
        drawerContent = {
            ModalDrawerSheet(
                drawerContainerColor = BgDeep // Uses your app's background color
            ) {
        ...
        // 4. Your existing Scaffold goes inside here
        Scaffold(
            ...
            NavHost(
                ...
                // ... Your existing composable routes remain exactly the same ...
                ...
                composable(Screen.Servers.route) { ServersScreen(onServerClick = { id -> navController.navigate(Screen.Terminal.createRoute(id)) }) }
                ...
                composable(
                    ...
                    TerminalScreen(serverId = serverId, onBack = { navController.popBackStack() })
                }
                composable(Screen.Diagnostics.route) { DiagnosticsScreen(onBack = { navController.popBackStack() }) }
                composable(Screen.Tunnel.route) { TunnelScreen(onBack = { navController.popBackStack() }) }
                composable(Screen.Academy.route) { AcademyScreen(onBack = { navController.popBackStack() }) }
            }
        }
    }
}
